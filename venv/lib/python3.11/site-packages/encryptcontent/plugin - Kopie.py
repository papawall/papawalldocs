...
SETTINGS = {
    'title_prefix': '[保护中] ',
    'summary': '这个页面受到密码保护,请输入密码',
    'placeholder': '使用回车确认密码',
    'placeholder_user': '用户名',
    'password_button_text': '解锁',
    'decryption_failure_message': '不对哦小老弟.',
    'encryption_info_message': '默认密码是:最小的4位数 木棍 二条 三杠 四条 防止爬虫'
}
...
class encryptContentPlugin(BasePlugin):
    ...
    def __encrypt_content__(self, content, base_path, encryptcontent_path, encryptcontent):
        ...
        #if 'sharelink' in encryptcontent:
        #    keystore.insert(0, 
    ...
    def on_config(self, config, **kwargs):
        ...
        if 'sharelinks' not in self.setup and self.config['sharelinks']:
            self.setup['sharelinks_file'] = self.setup['config_path'].joinpath(self.config['sharelinks_file'])
            self.setup['sharelinks'] = {}
            self.setup['sharelinks']['index'] = 1
            self.setup['sharelinks']['levels'] = {}
            self.setup['sharelinks']['urls'] = {}
            if self.setup['sharelinks_file'].exists():
                with open(self.setup['sharelinks_file'], 'r') as stream:
                    self.setup['sharelinks'] = yaml_load(stream)
    ...
    def on_page_context(self, context, page, config, **kwargs):
        ...
        if hasattr(page, 'encryptcontent'):
            ...
            # Gernerate sharelink keystore entry
            if 'sharelink' in page.meta.keys():
                if self.config['sharelinks'] and page.meta.get('sharelink'):
                    if page.url not in self.setup['sharelinks']:
                        self.setup['sharelinks'][page.url] = self.__get_random_password__()
                    page.encryptcontent['sharelink'] = self.setup['sharelinks'][page.url]
    ...
    def on_post_build(self, config, **kwargs):
        ...
        if self.config['sharelinks']:
            with open(self.setup['sharelinks_file'], 'w') as stream:
                yaml.dump(self.setup['sharelinks'],stream)
